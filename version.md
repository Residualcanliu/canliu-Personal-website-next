...
## v3.4 (2026-07-01) — 人马座星点收集游戏 + 排行榜

**人马座星图：**
- 新增 20 颗人马座星点 + 23 条连线（茶壶形），渲染在 Canvas 星空纹理
- 左中热区 + 导航栏「游戏」入口，独立路由 `/sagittarius`

**星点收集游戏（StarCollector）：**
| 模式 | 说明 |
|------|------|
| 计时模式 | 60 秒限时得分，接炸弹 -3 分，漏接不扣分 |
| 生命模式 | 3 条命，接炸弹 -1 命，漏接不扣分 |
| 深渊模式 | 自定义碗大小/下落速度/移速/星星大小，难度越高倍率越高（0.25x~3.45x） |
...
**狂热系统（全模式）：**
- 顶部进度条，接星累积（金 +1/蓝 +3），满 30 触发 7 秒狂热
- 狂热期间：无炸弹 + 3 倍星星密度雨

**排行榜系统：**
- `gameScore` 数据库表（mode/score/playerName/config）
- 右侧排行榜面板，切换深渊/计时/生命，深渊显示参数配置
- 成绩上传 + 同名检测 + 覆盖必须新分高于旧分

**游戏控制：** 纯键盘 A/D 或 ← →，Web Audio 合成音效（金/蓝/炸弹/技能/盾碎）
